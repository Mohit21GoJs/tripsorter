import PriorityQueue from 'packages/data-structures/priority-queue/PriorityQueue';
 ...
 */
export default function dijkstra(graph, startVertex, stopVertex) {
  ...
  distances[startVertex.getKey()] = {
    val: 0,
    edges: [],
  };

  // Init vertices queue.
  queue.add(startVertex, distances[startVertex.getKey()].val);
  ...
  while (!queue.isEmpty()) {
    ...
    currentVertex.getNeighbors().forEach(neighbor => {
      ...
      if (!visitedVertices[neighbor.getKey()]) {
        ...
        const existingDistanceToNeighbor = distances[neighbor.getKey()].val;
        // eslint-disable-next-line operator-linebreak
        const distanceToNeighborFromCurrent =
          distances[currentVertex.getKey()].val + edge.weight;
        ...
        if (distanceToNeighborFromCurrent < existingDistanceToNeighbor) {
          ...
          distances[neighbor.getKey()] = {
            val: distanceToNeighborFromCurrent,
            edges,
          };

          // Change priority of the neighbor in a queue since it might have became closer.
          if (queue.hasValue(neighbor)) {
            queue.changePriority(neighbor, distances[neighbor.getKey()].val);
          }
        ...
        if (!queue.hasValue(neighbor)) {
          queue.add(neighbor, distances[neighbor.getKey()].val);
        }
      }
    });
  ...
  return {
    distances,
    previousVertices,
    startToEnd: distances[stopVertex],
  };
}
